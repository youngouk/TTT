...
def show_video_processing_form():
    st.header("Process New YouTube Video")
    st.warning(f"Note: Currently, only videos under 30 minutes can be processed.")
        ...
        try:
            ...
            with st.spinner("Fetching video information... ⏳"):
                ...
                if duration > video_processing.MAX_VIDEO_DURATION:
                    st.error(f"This video is {duration // 60} minutes long. Only videos under 30 minutes can be processed.")
                    return
                estimated_time = (duration // 600) * 60 + (duration % 600) // 10  # Calculation based on 60 seconds per 10 minutes
                st.info(f"**{title}** ({channel}) - Estimated processing time: about {estimated_time} seconds ⏰")
            ...
            is_new_video = False
            if existing_video:
                st.info(f"This video has already been processed. Using existing data.")
                ...
                st.success(f"Video processing complete! 🎉 (Time taken: {video_processing.format_time(elapsed_time)})")
            ...
            with col1:
                st.page_link("pages/02_ask_question.py", label="Ask questions about this video", icon="💬")
            with col2:
                st.page_link("pages/03_video_list.py", label="View list of processed videos", icon="📋")
...
def main():
    ...
    if 'user' not in st.session_state or not st.session_state.user:
        st.warning("You need to log in to process videos.")
